function updated_locations = transform_pathpoints_usingTPS(rib_data, transformations, varargin)
    % Transform path points using centerline-based transformations and update OSIM file directly
    ...
    fprintf('Transforming %d path points using centerline-based method...\n', length(rib_data));
    ...
    for i = 1:length(rib_data)
        ...
        % Parse rib information - Using improved parsing from relative position method
        ...
        try
            % Apply linear blend transformation (or whatever centerline method you use)
